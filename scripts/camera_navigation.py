...
CLOSE_DISTANCE = 0.08 #originally 0.08
BACK_UP_DISTANCE = 0.11 #originally 0.11
def state_advancing():
    detection = camera_detections[destination]
    w = kp_control(-detection.x, 2, min_angular, max_angular)
...
def switch_searching():
    ...
    if destination in camera_detections:
        detection = camera_detections[destination]
        if my_time() - detection.time <0.1 and abs(detection.x)<0.5:
...
def switch_increase_distance():
    ...
    if distance_measures[1] > BACK_UP_DISTANCE:
